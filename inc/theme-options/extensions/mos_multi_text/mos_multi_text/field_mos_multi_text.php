 ...
 * @subpackage  Field_mos_multi_text
...
// Don't duplicate me!
if ( ! class_exists( 'ReduxFramework_mos_multi_text' ) ) {

    /**
     * Main ReduxFramework_mos_multi_text class
     *
     * @since       1.0.0
     */
    class ReduxFramework_mos_multi_text {
        ...
        public function render() {
            ...
            if ( isset( $this->value ) && is_array( $this->value ) ) {
                foreach ( $this->value as $k => $value ) {
                    if ( $value != '' || ($value == '' && $this->show_empty == true) ) {
                        echo '<li>';
                        echo     '<textarea id="' . $this->field['id'] . '-' . $k . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[]' . '" class="regular-text ' . $this->field['class'] . '">' . esc_attr( $value ) . '</textarea> ';
                ...
                echo '<li>';
                echo     '<textarea id="' . $this->field['id'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[]' . '" value="" class="regular-text ' . $this->field['class'] . '"></testarea> ';
                echo     '<a' . ' data-id="' . $this->field['id'] . '-ul"  href="javascript:void(0);" class="deletion redux-multi-text-remove">' . __( 'Remove', 'redux-framework' ) . '</a>';
                echo '</li>';
            }
            ...
            echo     '<li style="display:none;"><textarea id="' . $this->field['id'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[]' . '" class="regular-text" ></textarea> ';
            echo         '<a' . ' data-id="' . $this->field['id'] . '-ul" href="javascript:void(0);" class="deletion redux-multi-text-remove">' . __( 'Remove', 'redux-framework' ) . '</a>';
            echo     '</li>';
            echo '</ul>';
